...
import { AlertTriangle, ArrowUp, CheckCircle, X, ChevronLeft, ChevronRight, TrendingUp, Clock, Shield } from 'lucide-react';
...
const NewsSection = ({ news }: NewsProps) => {
  ...
  const getCategoryStyles = (category: string) => {
    ...
    return styles[category as keyof typeof styles] || "bg-gray-100 text-gray-700 hover:bg-gray-200";
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-12">
        <div className="relative mb-8 md:mb-0">
          ...
          <div className="relative">
            <span className="inline-block px-4 py-1.5 mb-3 text-xs font-medium text-blue-700 bg-blue-100 rounded-full">AI-Powered Analysis</span>
      ...
      >
        {news
          ...
          .map((newsItem: NewsItem, index: number) => {
            const isFake = newsItem.isFake === "true" || Number(newsItem.fake_percentage) > 50;
            ...
            const title = firstSentenceEnd > 0 ? reasonsText.substring(0, firstSentenceEnd + 1) : "News Analysis";
            ...
            return (
              ...
              >
                ...
                >
                  <div className="relative">
                    {/* Status Badge */}
                    <div className={`absolute top-3 left-3 z-10 rounded-full px-3 py-1.5 text-xs font-semibold text-white shadow-lg backdrop-blur-sm ${
                      isFake 
                    ...
                    >
                      <div className="flex items-center gap-1.5">
                        ...
                        <span>{isFake ? "Fake" : isHighlyCredible ? "Real" : "Misleading"}</span>
                      </div>
                    </div>

                    {/* Credibility Score */}
                    <div className={`absolute top-3 right-3 z-10 rounded-full px-3 py-1.5 text-xs font-semibold shadow-lg backdrop-blur-sm ${
                      isFake 
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="font-bold">
                          {isFake ? newsItem.fake_percentage : newsItem.real_percentage}%
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="w-full h-48 overflow-hidden">
                      ...
                      <img
                        ...
                        alt={isFake ? "Fake news illustration" : "Verified news illustration"}
                  ...
                  <CardContent className="p-5 flex flex-col gap-4 flex-grow relative">
                    ...
                    <div className="flex items-center justify-between">
                      <Badge className={`${getCategoryStyles(category)} px-3 py-1.5 text-xs font-medium rounded-full shadow-sm border border-current/10`}>
                        {category || 'General'}
                      </Badge>
                    ...
                    <div className="flex-grow">
                      <h3 className="text-lg font-semibold text-gray-900 group-hover:text-blue-600 
                        ...
                        {title?.length > 80 ? `${title.substring(0, 80)}...` : title || 'Untitled'}
                      </h3>
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-gray-100 mt-auto">
                      <TooltipProvider>
                        <Tooltip>
                          ...
                          <TooltipContent>
                            <p>{newsItem.author_verified === "true" ? "Verified Source" : "Unverified Source"}</p>
                          </TooltipContent>
                        </Tooltip>
                        ...
                        <Tooltip>
                          <TooltipTrigger>
                            <button
                              ...
                              className={`flex items-center gap-2 rounded-full px-4 py-1.5 text-sm font-medium transition-all duration-300 ${
                                currentUser && votes[newsItem.id]?.includes(currentUser?.id)
                          ...
                          <TooltipContent>
                            <p>{currentUser ? "Vote for credibility" : "Login to vote"}</p>
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    </div>
              ...
              <PaginationContent>
                <PaginationItem>
                  <Button
                    ...
                    className={`rounded-lg transition-all duration-300 ${currentPage === 1 ? 'opacity-50' : 'hover:bg-blue-50 hover:text-blue-600'}`}
                  >
                ...
                {Array.from({ length: Math.ceil(news.length / ITEMS_PER_PAGE) }, (_, i) => {
